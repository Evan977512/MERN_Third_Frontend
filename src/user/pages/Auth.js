  ...
  const authSubmitHandler = async (event) => {
    ...
    if (isLoginMode) {
      ...
      try {
        ...
        const responseData = await sendRequest("http://localhost:5001/api/users/signup", "POST", formData);
        auth.login(responseData.userId, responseData.token);
      } catch (err) {}
    }
  };
    ...
    <React.Fragment>
      ...
      <Card className="authentication">
        ...
        <form onSubmit={authSubmitHandler}>
          ...
          {!isLoginMode && (
            <ImageUpload center id="image" onInput={inputHandler} errorText="please input valid image" />
          )}
          <Input
            element="input"
            id="email"
